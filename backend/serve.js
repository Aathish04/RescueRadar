...
const main = async () => {  
  ...
  if (process.argv[2]) {
    db = await orbitdb.open(process.argv[2], { type: 'documents', AccessController: IPFSAccessController({ write: ['*'] }) })
  } else {
    ...
    db = await orbitdb.open('my-db', { type: 'documents', AccessController: IPFSAccessController({ write: ['*'] }) })
